...
data = pd.read_csv(r"C:\Users\Clarisa Hilario\PycharmProjects\pythonProject1\updated1.csv")
...
@app.callback(
    ...
    Output('age-graph', 'figure'),  # age
    Output('age-analysis', 'children'),
    Output('sex-graph', 'figure'),  # sex
...
def update_mainsource_graph(region):
    ...
    fig4 = px.strip(filt_data, x="Total Number of Family members", y="total_expenditures",
    ...
    numexp_analysis = 'In ' + region + ', the highest total expenditures of ' + exp_max + ' and houses a total of ' + num_max + ' members.'

    # age
    age = filt_data['Household Head Age'].value_counts().rename_axis('Household Head Age').reset_index(name='counts')
    fig5 = px.strip(age, x="Household Head Age", y="counts",
                    title="Household Head Age in " + region)
    fig5.update_layout(transition_duration=1000, title="Household Head Age in " + region)

    # age analysis
    age_max = age['counts'].idxmax()
    age_min = age['counts'].idxmin()
    age_majority = str(age['Household Head Age'][age_max])
    age_minority = str(age['Household Head Age'][age_min])
    age_analysis = 'In '+region+', majority of the household heads are in the age of'+ age_majority+' years old and the minority are in the age of '+age_minority+' years old.'

    # sex graph
    sex = filt_data['Household Head Sex'].value_counts().rename_axis('Household Head Sex').reset_index(name='counts')
    fig6 = px.pie(sex, values='counts', names='Household Head Sex', title='Household Head Sex in ' + region)
    fig6.update_layout(transition_duration=1000, title="Household Head Sex in " + region)

    # sex analysis
    sex_max = sex['counts'].idxmax()
    sex_min = sex['counts'].idxmin()
    sex_majority = sex['Household Head Sex'][sex_max]
    sex_minority = sex['Household Head Sex'][sex_min]
    sex_analysis = 'The sex of majority of the household heads in ' + region + ' are ' + sex_majority + ' and the minority are '+sex_minority+'.'

    return fig1, ms_analysis, fig2, occ_analysis, fig3, stat_analysis, fig4, numexp_analysis, fig5, age_analysis, fig6, sex_analysis
